...
export class AdminButton extends Component {
    ...
    get contractType() {
        return this.props.contractType || null;
    }

    get adapter() {
        return this.props.adapter || null;
    }

    get projectData() {
        return this.props.projectData || null;
    }

    async didMount() {
        console.log('[AdminButton] didMount called', {
        ...
        const unsub2 = eventBus.on('wallet:changed', () => this.checkOwnership());
    ...
    async openAdminModal() {
        if (!this._modal) {
            ...
            // Dynamic import of appropriate modal
            // Using original modals since they haven't been migrated yet
            if (isERC1155) {
                stylesheetLoader.load('src/components/AdminDashboard/erc1155-admin.css', 'erc1155-admin-styles');

                const { ERC1155AdminModal } = await import('../AdminDashboard/ERC1155AdminModal.js');
                this._modal = new ERC1155AdminModal(
                    this.contractAddress,
                    this.contractType,
                    this.adapter,
                    this.projectData
                ...
                const { ERC404AdminModal } = await import('../AdminDashboard/ERC404AdminModal.js');
                this._modal = new ERC404AdminModal(
                    this.contractAddress,
                    this.contractType,
                    this.adapter,
                    this.projectData
                ...
                const { AdminDashboardModal } = await import('../AdminDashboard/AdminDashboardModal.js');
                this._modal = new AdminDashboardModal(
                    this.contractAddress,
                    this.contractType,
                    this.adapter
                );
            ...
            this._modal.mount(modalContainer);
        }
    ...
    cleanupModal() {
        if (this._modal) {
            this._modal.close();
            if (typeof this._modal.unmount === 'function') {
                this._modal.unmount();
            }
    ...
    render() {
        ...
        if (loading || !isOwner) {
            return h('div', { className: 'admin-button-container' });
        }
        ...
        return h('button', {
            ...
            h('span', { className: 'admin-button-icon' }, '♚'),
            h('span', { className: 'admin-button-text' }, 'Admin')
        );
    }
}
