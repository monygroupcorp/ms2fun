...
- The **destination is curation, not code** — the protocol owner can repoint a target's payout address or retire the target. The ratio is the promise; the payee is a listing decision you can watch and leave over. If a target is retired after you bind, the share taken at **settlement** is **returned to you**, never to the protocol. The ERC-404 pool's swap charge is the one exception, and it goes the other way: retiring the target lets anyone switch that charge off, and the ETH then stays with the trader rather than coming to you.
...
**The venue decides whether the community keeps earning.** Only a **Uniswap V4** pool can carry an alignment hook, which taxes the ETH side of every swap — buys and sells alike — into the vault for as long as that vault stays curated. If the protocol retires the vault, anyone can halt the charge and swaps stop paying it; anyone can start it back up if the vault is curated again. ZAMM graduates into an untaxed pool: there the graduation 19% is the whole of the community's take, and seeding depth on a venue the community does not already depend on is the service instead. The hook is a protocol-level switch rather than a creator setting.
