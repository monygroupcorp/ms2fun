...
import { fetchJson, metadataOrNull, type CollectionMetadata, type AllowlistRow } from '../metadata'
...
export async function buildAllowlistFromUri(
  ...
  const json = metadataOrNull(await fetchJson(trimmed, signal))
...
export async function resolveMemberProof(
  ...
  const json = metadataOrNull(await fetchJson(listURI, signal))
