...
contract ScanAlignmentPools is Script {
    ...
    function run(address token) external view {
        ...
        if (best.found) {
            ...
            console.log(
                "  -> wire setVaultPoolKey(vault, {currency0: ETH, currency1: token, fee, tickSpacing, hooks: 0})"
            );
        } else {
    ...
    function _scanZamm(address token) internal view {
        ...
        for (uint256 i = 0; i < ZAMM_FEES.length; i++) {
            IZAMM.PoolKey memory key =
                IZAMM.PoolKey({ id0: 0, id1: 0, token0: address(0), token1: token, feeOrHook: ZAMM_FEES[i] });
    ...
    function _symbol(address token) internal view returns (string memory) {
        try IERC20Decimals(token).symbol() returns (string memory s) {
            return s;
        } catch {
            return "?";
        }
