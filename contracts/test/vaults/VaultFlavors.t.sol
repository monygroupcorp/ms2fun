...
contract VaultFlavorsTest is Test {
    ...
    function _assertRegistered(address vault, uint256 targetId) internal view {
        IMasterRegistry.VaultInfo memory info =
            MasterRegistryV1(s.masterRegistry()).getVaultInfo(vault);
    ...
    function _ready(address vault) internal view returns (bool ok) {
        (, bytes memory ret) =
            vault.staticcall(abi.encodeWithSignature("isLiquidityReady()"));
    ...
    function test_zammPoolKeyWiringGate() public {
        ZAMMAlignmentVaultFactory f = new ZAMMAlignmentVaultFactory(
            STUB_ZAMM, address(1), address(2), IVaultPriceValidator(address(3))
        );
        ...
        IZAMM.PoolKey memory key = IZAMM.PoolKey({
            id0: 0, id1: 0, token0: address(0), token1: STUB_LINK, feeOrHook: 100
        });
