...
contract CypherAlignmentVaultTest is Test {
    ...
    // ── Off-price pool is a HARD revert, full stop (rth-locked: no owner override, no self-spot) ─────
    // Algebra createPool+initialize are permissionless, so a griefer can pre-seed the vault's
    // target/WETH pool at a garbage price. By design the vault reverts LpPoolPriceDeviation on EVERY
    // convert and never stores lpPool — there is deliberately NO setLpPool escape hatch and NO
    // self-spot fallback. The griefer-created-off-price DoS is a KNOWN, SEPARATE follow-up spec; it is
    // NOT solved here by inventing a trust surface (gauntlet-locked, noesis-027b).

    function test_convert_offPricePoolRevertsPermanently_noEscape() public {
        _contribute(alice, 10 ether);
        _fundAcquire(1000 ether, 1e18);

        // Griefer: createPool + initialize at 2x the reference (>5% off).
        address pool = factory.createPool(address(alignmentToken), address(weth), "");
        MockAlgebraPool(pool).initialize(uint160(uint256(_refSqrt(ETH_PER_TOKEN)) * 2));

        // The revert is permanent: convert reverts, no pool is stored, and it stays bricked on retry —
        // the reference is the sole price authority, so an off-price pool is never accepted.
        vm.expectRevert(CypherAlignmentVault.LpPoolPriceDeviation.selector);
        vault.convertAndAddLiquidity(0);
        assertEq(vault.lpPool(), address(0), "no pool stored on an off-price pool");
        assertEq(vault.totalPendingETH(), 10 ether, "tithe unchanged (tx fully reverted)");

        // Re-running still reverts — there is no state that could have loosened the guard.
        vm.expectRevert(CypherAlignmentVault.LpPoolPriceDeviation.selector);
        vault.convertAndAddLiquidity(0);
    }
