...
contract ERC721AgentDelegationTest is GlobalMessagingTestBase {
    ...
    function _params(string memory _name, address _creator)
        internal
        view
        returns (ERC721AuctionFactory.CreateParams memory)
    {
    ...
    function test_agent_creates_instance_on_behalf() public {
        ...
        vm.prank(agent);
        address instance = factory.createInstance{ value: 0 }(_nextSalt(), _params("Agent Auction", artist));
    ...
    function test_self_created_delegation_disabled() public {
        ...
        vm.prank(artist);
        address instance = factory.createInstance{ value: 0 }(_nextSalt(), _params("Self Auction", artist));
    ...
    function test_agent_queues_piece_directly() public {
        ...
        vm.prank(agent);
        address instance = factory.createInstance{ value: 0 }(_nextSalt(), _params("Queue Test", artist));
    ...
    function test_only_owner_can_set_delegation() public {
        vm.deal(artist, 1 ether);
        vm.prank(artist);
        address instance = factory.createInstance{ value: 0 }(_nextSalt(), _params("Auth Test", artist));
