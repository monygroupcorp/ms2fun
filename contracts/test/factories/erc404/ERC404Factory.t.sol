...
/// @dev Vault that self-reports an arbitrary `vaultType()`.
contract TypedVault {
...
/// @dev Vault whose `vaultType()` REVERTS. Nothing in the create path reads it, so this must create.
...
contract ERC404FactoryTest is Test {
    ...
    // ── Vault family is not a create-time gate ────────────────────────────────
    // ERC404 creation used to refuse an endowment-family vault, because graduation split family-blind
    // (1/19/80) while mint settlement split the endowment's way (1/80/19), so the pairing would have
    // routed the endowment's own leg past it. Intake is family-blind everywhere now — one split, every
    // family — so the refusal has nothing left to protect and the factory reads no `vaultType()` here at
    // all. What still curates which vaults may be used is the master registry, and only that.
    ...
    /// @notice An `AaveEndowment`-family vault CREATES. This is the pairing the factory used to refuse.
    function test_createInstance_acceptsEndowmentFamilyVault() public {
        address instance = _createWithVault("EndowToken", address(new TypedVault("AaveEndowment")));
        assertTrue(instance != address(0), "an endowment vault must create like any other");
    }
    ...
    /// @notice An UNKNOWN `vaultType()` creates, and so does one that REVERTS — the create path asks no
    ///         vault what family it is, so there is nothing here for a hostile or future answer to brick.
    function test_createInstance_doesNotConsultVaultTypeAtAll() public {
        assertTrue(
            _createWithVault("FutureVaultToken", address(new TypedVault("SomeFutureVenueLP"))) != address(0),
            "unknown vaultType must not block create"
        );
        assertTrue(
            _createWithVault("RevertTypeToken", address(new RevertingTypeVault())) != address(0),
            "reverting vaultType must not block create"
        );
    }
}
