...
### Post-Graduation Swap Tax (`UniAlignmentV4Hook`)

A graduated ERC404 pool is a Uniswap V4 pool wired to `UniAlignmentV4Hook`, and every swap through it pays an ETH-side tithe straight to the alignment vault. The rate is `hookFeeBips`, fixed immutably on each hook when that hook is deployed and chosen per deployment rather than by this document: the module ships with `hookFeeBips == 0` **and** `alignmentHookFactory == address(0)`, so graduation takes the untaxed static-fee path until an operator seeds a rate and then turns the factory on as a separate governed op (`setAlignmentHookFactory`). There is therefore no default rate to quote — an integrator sizes the tithe by reading `hookFeeBips()` off the pool's own hook, and the 1% that runs through this codebase's fixtures is the fixture's figure, not a value the code sets. Which callback charges it depends on which side of V4's specified/unspecified divide the ETH leg falls on: `beforeSwap` charges the two shapes where ETH is the **specified** currency — an exact-input ETH buy and an exact-output ETH sell — and `afterSwap` charges the two where ETH is **unspecified**. An exact-output swap still receives exactly the ETH it named; the tithe widens what that swap costs on the other side rather than shrinking what it delivers.

**Integrating: name the extreme sqrt-price sentinels.** The two shapes charged in `beforeSwap` are priced on the ETH the caller *named*, because V4 fixes a hook's credit on the specified currency before the swap runs. `Pool.swap` then stops at `sqrtPriceLimitX96` and returns only what it moved, so a leg filled in part would leave the caller paying a fee priced on ETH that never moved — and no callback can hand that ETH back, since `afterSwap` may only adjust the unspecified currency. The hook refuses the trade rather than overcharging it: `afterSwap` re-reads the realised delta and reverts **`NamedEthLegNotFilled`** unless the pool moved the whole ETH leg the fee was priced against.

A swap against an alignment-hooked pool must therefore pass the extreme sentinel for its direction — `TickMath.MIN_SQRT_PRICE + 1` when `zeroForOne`, `TickMath.MAX_SQRT_PRICE - 1` otherwise — so that nothing but exhausted liquidity can stop it short. A caller that instead sets a binding `sqrtPriceLimitX96` to cap price impact, or that splits one order across several pools, is **reverted rather than partially filled** on those two shapes. This is a deliberate difference from an unhooked V4 pool, where the same swap comes back part-filled. The protocol's own swap path, `AlignmentHookSwapRouter`, passes the sentinels, so the rule bears on anything that builds its own `SwapParams`. The two ETH-unspecified shapes are charged on the realised delta and are unaffected — they part-fill as usual — as is any swap the hook takes no fee on, whether because the tithe rounds to zero or because it has been halted.
