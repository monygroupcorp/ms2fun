...
contract CypherAlignmentVault is IAlignmentVault, Ownable, ReentrancyGuard {
    ...
    ///      `maxPriceDeviationBps` — the pool was pushed/manipulated; retry once it re-converges. Full
    ///      stop: there is NO owner override and NO self-spot fallback (rth-locked, noesis-027b). The
    ///      griefer-created-off-price DoS is a KNOWN, SEPARATE follow-up — never solved by a trust surface here.
    ...
    /// @dev Resolve the target/WETH Algebra pool (Algebra is one-pool-per-pair), creating and seeding it
    ///      at the reference price when absent (or when present-but-uninitialized). An already-initialized
    ///      pool is accepted ONLY within `maxPriceDeviationBps` of the reference — a manipulated/pushed
    ///      pool reverts `LpPoolPriceDeviation`, full stop (no owner override, no self-spot fallback).
    ///      Sets `lpPool`/`tokenIsZero` on first resolution. Returns the sqrtPrice the swap proportion is
    ///      sized against (the reference for a fresh/just-seeded pool; the pool's own vetted spot otherwise).
    // slither-disable-next-line reentrancy-benign
    function _resolveOrCreatePool(uint160 referenceSqrtPrice) private returns (uint160 validatedSqrtPrice) {
        address pool = IAlgebraFactory(algebraFactory).poolByPair(alignmentToken, weth);
        bool created;
        if (pool == address(0)) {
    ...
    /// @dev A previously-created pool: initialize a still-fresh one (spot 0) at the reference; otherwise
    ///      require its spot within `maxPriceDeviationBps` of the reference, reverting otherwise. There is
